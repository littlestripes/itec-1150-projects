...
def main():
    filename_re = re.compile(r"/(\d\w)+$")  # grabs whatever's at the end of the URL
    ...
    str_url = _input
    # Set filename
    str_filename = filename_re.search(str_url)
    str_filename = f"{str_filename}.txt"  # set filename to save contents to
    ...
    while True:
        print("Care for another go, chap? (y or n)")
        choice = str(input("# "))
        if choice == "y":
            main()
        else:
            print("Good night, America!")
            break
